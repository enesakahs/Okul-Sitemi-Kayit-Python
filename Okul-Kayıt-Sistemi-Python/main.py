...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        ...
        try:
            ...
            connector.commit()
            mb.showinfo('Kayıt Eklendi', f"Kayıt {name} basarıyla eklendi")
            reset_fields()
            display_records()
        except:
            mb.showerror('Yanlış veri tipi', 'Girilen değerlerin türü doğru değil. Lütfen iletişim alanının yalnızca sayıları içerebileceğini unutmayın.')
...
def remove_record():
    if not tree.selection():
        ...
        mb.showinfo('Basarılı','Silinmesini istediğiniz kayıt başarıyla silinmiştir.')
...
main = Tk()
main.title('Okul Yönetim Sistemi')
main.geometry('1000x600')
main.resizable(0, 0)

# Creating the background and foreground color variables
lf_bg = 'darkred' # bg color for the left_frame
cf_bg = 'lightblue' # bg color for the center_frame
...
Label(main, text="OKUL YÖNETİM SİSTEMİ", font=headlabelfont, bg='limegreen').pack(side=TOP, fill=X)
...
Label(left_frame, text="İletisim Numarası", font=labelfont, bg=lf_bg).place(relx=0.175, rely=0.18)
Label(left_frame, text="Email Adresi", font=labelfont, bg=lf_bg).place(relx=0.2, rely=0.31)
Label(left_frame, text="Cinsitet", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.44)
Label(left_frame, text="Dogum Günü", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.57)
...
OptionMenu(left_frame, gender_strvar, 'Erkek', "Kadın").place(x=45, rely=0.49, relwidth=0.5)

dob = DateEntry(left_frame, font=("Arial", 12), width=15)
dob.place(x=20, rely=0.62)

Button(left_frame, text='Kayıt Ekle', font=labelfont, command=add_record, width=18).place(relx=0.025, rely=0.85)

# Placing components in the center frame
Button(center_frame, text='Kayıt Sil', font=labelfont, command=remove_record, width=15).place(relx=0.1, rely=0.25)
Button(center_frame, text='Kayıt Görüntüle', font=labelfont, command=view_record, width=15).place(relx=0.1, rely=0.35)
Button(center_frame, text='Sıfırla', font=labelfont, command=reset_fields, width=15).place(relx=0.1, rely=0.45)
Button(center_frame, text='Veritabanı Sil', font=labelfont, command=reset_form, width=15).place(relx=0.1, rely=0.55)

# Placing components in the right frame
Label(right_frame, text='Öğrenci Kayıtları', font=headlabelfont, bg='lightgray', fg='darkgray').pack(side=TOP, fill=X)
...
tree.heading('Name', text='İsim', anchor=CENTER)
tree.heading('Email Address', text='Email', anchor=CENTER)
tree.heading('Contact Number', text='Telefon', anchor=CENTER)
tree.heading('Gender', text='Cinsiyet', anchor=CENTER)
tree.heading('Date of Birth', text='DGT', anchor=CENTER)
